 ...
 */
static void do_inject_memory_buffer_bf(hwaddr *value,
																				int injected_bit)
{
 ...
 * @param[in] fi_info - information for performing faults.
 */
static void do_inject_memory_buffer_rs(hwaddr *value,
																			FaultInjectionInfo fi_info)
{
	if (fi_info.bit_value)
 ...
 * @param[in] fi_info - information for performing faults.
 */
static void do_inject_new_memory_value_buffer(hwaddr *orig_value,
																					FaultInjectionInfo fi_info)
{
 ...
 * @param[in] injected_bit - the position of the affected bit.
 */
static void do_inject_memory_bf(CPUArchState *env, hwaddr inject_address,
																	int injected_bit)
{
     CPUState *cpu = ENV_GET_CPU(env);
 ...
 * @param[in] injected_bit - the position of the affected bit.
 */
static void do_inject_memory_rs(CPUArchState *env, hwaddr inject_address,
																	FaultInjectionInfo fi_info)
{
    CPUState *cpu = ENV_GET_CPU(env);
 ...
 * @param[in] injected_bit - the position of the affected bit.
 */
static void do_inject_new_memory_value(CPUArchState *env,  hwaddr inject_address,
																				FaultInjectionInfo fi_info)
{
    CPUState *cpu = ENV_GET_CPU(env);
 ...
 * @param[in] fi_info - information for performing faults.
 */
static void do_inject_memory_buffer(hwaddr *addr,
																FaultInjectionInfo fi_info)
{
	if (fi_info.bit_flip)
		do_inject_memory_buffer_bf(addr, fi_info.injected_bit);
	else if (!fi_info.bit_flip && !fi_info.new_value)
		do_inject_memory_buffer_rs(addr, fi_info);
	else if (!fi_info.bit_flip && fi_info.new_value)
		do_inject_new_memory_value_buffer(addr, fi_info);
}
 ...
 * @param[in] fi_info - information for performing faults.
 */
static void do_inject_memory(CPUArchState *env, hwaddr *addr,
															FaultInjectionInfo fi_info)
{
	if (fi_info.bit_flip)
		do_inject_memory_bf(env, *addr, fi_info.injected_bit);
	else if (!fi_info.bit_flip && !fi_info.new_value)
		do_inject_memory_rs(env, *addr, fi_info);
	else if (!fi_info.bit_flip && fi_info.new_value)
		do_inject_new_memory_value(env, *addr, fi_info);
}
														...
														FaultInjectionInfo fi_info)
{
	if (fi_info.fault_on_register)
	{
		if (fi_info.fault_on_address || fi_info.access_triggered_content_fault)
			do_inject_memory_buffer(addr, fi_info);
		else {
			#if defined(TARGET_ARM)
	...
	else
	{
		if (fi_info.fault_on_address || fi_info.access_triggered_content_fault)
			...
			do_inject_memory(env, addr, fi_info);
	}
}
